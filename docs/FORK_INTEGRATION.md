...
| Nexus updates | Manual, paced file-replacement checks with a 24-hour project cache, verified local download recovery, project-only listings, and resettable package-hash-bound reference releases. Package metadata and download versions are distinguished. See [manual update checking](FORK_NEXUS_UPDATES.md). |
