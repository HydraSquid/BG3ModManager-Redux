...
public sealed class NexusDownloadedModValidationException : IOException
{
	...
	internal NexusDownloadedModValidationException(string code, string details,
		IReadOnlyList<DivinityModData> inspectedMods = null) : base(details)
	{
		ErrorCode = code;
	...
	public static (string Code, string Details) Describe(Exception error) => error switch
	{
		...
		NexusDownloadedModRollbackException => ("rollback-failed", "Some installed files could not be restored. Do not retry installation until the affected files have been recovered."),
		FileNotFoundException or DirectoryNotFoundException => ("missing-file", "A required file or folder could not be found. Check that the downloaded archive and configured Mods folder are available."),
...
public sealed class NexusDownloadedModImporter
{
	...
	{
		var availableMods = (installedMods ?? []).ToArray();
	...
	public async Task<NexusDownloadedModImportTransaction> StageAsync(string packagePath, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			foreach (var package in stagedPaks)
			{
				try
				{
					await _preflight(package.StagedPath,
						stagedPaks.Where(other => other != package).Select(other => other.Mod).ToArray(), cancellationToken);
				}
		...
		string sourcePath, string stagingRoot, CancellationToken cancellationToken)
	{
		...
		if (entries.Length > MaximumArchiveEntries)
			throw new NexusDownloadedModValidationException("archive-layout", "The archive exceeds the safety limit of 4096 entries.");
		...
		{
			cancellationToken.ThrowIfCancellationRequested();
			var path = Path.Combine(stagingRoot, names[index]);
			...
			result.Add((names[index], path));
		}
		return result;
	}

	private static async Task<long> CopyBoundedAsync(Stream source, string destination, long maximumBytes,
...
public sealed class NexusDownloadedModImportTransaction : IAsyncDisposable
{
		...
		Func<StagedNexusPak, Task<DivinityModData>> register, CancellationToken cancellationToken)
	{
		...
		try
		{
			foreach (var package in _packages)
			{
				...
				else File.Move(package.StagedPath, package.DestinationPath);
	...
	/// <summary>
	/// Commits staged files and applies the caller's real library registration as
	/// one boundary. A registration failure restores both the files and the
	/// caller-provided in-memory snapshot before surfacing the failure.
	/// </summary>
	public async Task<IReadOnlyList<DivinityModData>> CommitAndRegisterAsync(
		Func<StagedNexusPak, Task<DivinityModData>> register,
		Func<IReadOnlyList<DivinityModData>, Task> registerModels,
		Func<Task> restoreModels,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(registerModels);
		ArgumentNullException.ThrowIfNull(restoreModels);
		var models = await CommitAsync(register, cancellationToken);
		try
		{
