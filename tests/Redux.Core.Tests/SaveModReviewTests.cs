...
public sealed class SaveModReviewTests
{
    ...
    public void SaveRowMenuUsesClickedSaveAndExistingReviewState()
    {
        ...
        typeof(ReduxSaveManagerWindow).GetMethod("SaveRow_Loaded", flags)!.Invoke(window, new object[] { row, new System.Windows.RoutedEventArgs() });
        var menu = row.ContextMenu;
        menu.PlacementTarget = row;
        menu.RaiseEvent(new System.Windows.RoutedEventArgs(System.Windows.Controls.ContextMenu.OpenedEvent));
        RegressionAssert.True(ReferenceEquals(second, list.SelectedItem));
        RegressionAssert.Equal(3, menu.Items.Count);
        RegressionAssert.True(((System.Windows.Controls.MenuItem)menu.Items[0]).IsEnabled);
