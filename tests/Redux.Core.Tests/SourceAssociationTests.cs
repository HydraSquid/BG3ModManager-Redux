...
public sealed class SourceAssociationTests
{
	...
	public void NexusImportPersistsManualModioRemovalBeforeReload()
	{
		var mod = CreateMod();
		var nexusCache = new NexusModsCachedData();
		var modioCache = new ModioCachedData();
		modioCache.Mods[mod.UUID] = new ModioModData
		{
			UUID = mod.UUID,
			ModId = 6197684,
			Name = "Manual mod.io project",
			MetadataOrigin = ModioMetadataOrigin.Manual
		};

		modioCache.Mods.Remove(mod.UUID);
		mod.NexusModsData.SetModVersion(23751, 123);
		mod.NexusModsData.MetadataOrigin = NexusMetadataOrigin.NexusArchiveImport;
		nexusCache.Mods[mod.UUID] = mod.NexusModsData;

		var reloadedNexus = RoundTrip(nexusCache);
		var reloadedModio = RoundTrip(modioCache);
		var reloaded = CreateMod();
		RegressionAssert.False(reloadedModio.Mods.ContainsKey(reloaded.UUID));
		reloaded.NexusModsData.Update(reloadedNexus.Mods[reloaded.UUID]);

		RegressionAssert.Equal(NexusMetadataOrigin.NexusArchiveImport, reloaded.NexusModsData.MetadataOrigin);
		RegressionAssert.Equal(23751L, reloaded.NexusModsData.ModId);
		RegressionAssert.Equal(ModSourceType.NEXUSMODS, reloaded.Metadata.SourceType);
	}
