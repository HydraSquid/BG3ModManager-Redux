...
public static class Program
{
	...
	private static async Task<int> AddCommandAsync(CommandOptions options)
	{
		...
		var fileId = options.RequiredPositiveOrUnknownLong("file-id");
	...
	private static ValidationResult ValidateDatabase(JsonObject database)
	{
		...
		var modioProjects = database["modioProjects"] as JsonArray ?? new JsonArray();
		var modioProjectIds = new HashSet<long>();
		foreach (var project in modioProjects.OfType<JsonObject>())
		{
			var modId = GetLong(project, "modId");
			if (modId <= 0) result.Errors.Add("A mod.io project has a missing or invalid modId.");
			else if (!modioProjectIds.Add(modId)) result.Errors.Add($"Duplicate mod.io project modId {modId}.");
			if (String.IsNullOrWhiteSpace(GetString(project, "name"))) result.Errors.Add($"mod.io project {modId} has no name.");
			if (String.IsNullOrWhiteSpace(GetString(project, "nameId"))) result.Errors.Add($"mod.io project {modId} has no nameId.");
		}
		...
		var communityModioModules = database["communityModioIdentities"] as JsonArray ?? new JsonArray();
		var communityModioUuids = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
		foreach (var module in communityModioModules.OfType<JsonObject>())
		{
			var uuid = GetString(module, "uuid")?.Trim();
			var modId = GetLong(module, "modId");
			if (!Guid.TryParse(uuid, out _)) result.Errors.Add($"Invalid community mod.io module UUID '{uuid}'.");
			if (!modioProjectIds.Contains(modId)) result.Errors.Add($"Community mod.io module UUID {uuid} references missing project {modId}.");
			if (!String.Equals(GetString(module, "matchBasis"), "community-exact-name", StringComparison.Ordinal))
				result.Errors.Add($"Community mod.io module UUID {uuid} has an unsupported match basis.");
			if (!String.IsNullOrWhiteSpace(uuid) && !communityModioUuids.Add(uuid)) result.Errors.Add($"Duplicate community mod.io UUID {uuid}.");
			if (!String.IsNullOrWhiteSpace(uuid) && moduleProjects.ContainsKey(uuid))
				result.Errors.Add($"Module UUID {uuid} has both Nexus and mod.io community identities.");
		}
		...
		else
		{
			...
			if (database.ContainsKey("modioProjects") || counts.ContainsKey("modioProjects"))
				CheckCount(counts, "modioProjects", modioProjects.Count, result);
			...
			if (database.ContainsKey("communityModioIdentities") || counts.ContainsKey("communityModioIdentities"))
				CheckCount(counts, "communityModioIdentities", communityModioModules.Count, result);
	...
	private static void UpdateCounts(JsonObject database)
	{
		...
		counts["modioProjects"] = (database["modioProjects"] as JsonArray)?.Count ?? 0;
		...
		counts["communityModioIdentities"] = (database["communityModioIdentities"] as JsonArray)?.Count ?? 0;
...
""");
	...
	private sealed class CommandOptions
	{
		...
		public long RequiredPositiveOrUnknownLong(string key) =>
			long.TryParse(Required(key), out var value) && (value > 0 || value == -1)
				? value
				: throw new ArgumentException($"--{key} must be a positive integer or -1 when unknown.");
